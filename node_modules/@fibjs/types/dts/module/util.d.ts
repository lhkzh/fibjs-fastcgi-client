...
declare module 'util' {
    ...
    /**
     * @description 从一个构造函数 constructor 继承原型方法到另一个。构造函数的原型将被设置为一个新的从超类（superConstructor）创建的对象。
     * 
    ...
    /**
     * @description 方法返回 obj 的字符串表示，主要用于调试。 附加的 options 可用于改变格式化字符串的某些方面。
     * 
     *      @param obj 指定需要处理的对象
    ...
    /**
     * @description 包裹 callback 或 async 方法为同步调用
     * 
